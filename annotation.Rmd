...
Variants called from the TOPMed data set are annotated using the [Whole Genome Sequence Annotator (WGSA)](https://sites.google.com/site/jpopgen/wgsa). Output files from WGSA version 0.8 include more than 350 annotation fields annotating indel variants and 400 annotation fields annotating snv variants. In each case, some annotation fields are themselves lists of annotation values. (For example, a variant may be annotated with having different effects on different transcripts of a gene, or differing effects on different genes). Thus, individual variants may be annotated with more than 1000 fields. Not all of these fields will be useful for a particular analysis, and some may be mutually useful at the same time, so analysts need to choose which annotations they'd like to use, and then parse the WGSA output to select and format the annotation fields prior to filtering and aggregating for aggregate association tests.
...
The TOPMed DCC uses an R package we developed, *WGSAParsr*, to work with WGSA output files. Briefly, WGSAParsr simplifies the WGSA output files by: 1) selecting annotation fields; 2) renaming fields; and 3) simplifying fields that have compound list-entries. The specific details for each of these operations are specified by the user via a configuration file that details the desired operations for *WGSAParsr*.  The *WGSAParsr* package is under development, and is available on github at [https://github.com/UW-GAC/wgsaparsr](https://github.com/UW-GAC/wgsaparsr). It can be installed using the *devtools* package, like this:
```{r eval=FALSE}
...
*note*: if you get an error `Installation failed: error in running command` when you're trying to use `install_github()`, that may be related to some assumptions devtools makes in downloading packages from github. That error can be resolved if you set the unzip option in your R session before running `devtools::install_github()`. Here's how: `options(unzip = "internal")`
...
`parse_to_file()` function arguments include a path to a WGSA annotation file, a user-defined configuration file, output file destinations, along with other optional arguments. `parse_to_file()` reads the specified input annotation file in chunks, processes them following the specification defined in the configuration file, and writes the parsed, simplified annotation to the output destination(s). `parse_to_file()` produces a tab-separated output file useful for subsequent analysis.
